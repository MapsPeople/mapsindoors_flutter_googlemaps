...
class MapWidgetState extends State<MapWidget> {
  ...
  void doMenuItemAction(dynamic menuItem) async {
    if (menuItem is MPCategory) {
      ...
      final filterBuilder = MPFilter.builder()..setCategories(<String>[menuItem.key]);
      if (selectedVenueId != null) {
        filterBuilder.setParents([selectedVenueId!]);
        filterBuilder.setDepth(4);
      }
      List<MPLocation>? locs = await getLocationsByQuery(query: query, filter: filterBuilder.build());
      if (locs != null) {
        menuItems.clear();
              ...
              children: [
                ...
                Expanded(
                  child: DropdownButton(
                    items: const <DropdownMenuItem>[DropdownMenuItem(value: true, child: Text("true")), DropdownMenuItem(value: false, child: Text("false"))],
                    onChanged: (value) {
                      persistentBottomSheetController?.setState!(
                        () {
  ...
  Future<String> getDisplayRuleValue(String value, MPDisplayRule displayRule) async {
    switch (value) {
      case "icon":
  ...
  Future<bool> getBooleanDisplayRuleValue(String value, MPDisplayRule displayRule) async {
    switch (value) {
      case "visible":
  ...
  Future<num> getNumericalDisplayRuleValue(String value, MPDisplayRule displayRule) async {
    switch (value) {
      case "zoomFrom":
